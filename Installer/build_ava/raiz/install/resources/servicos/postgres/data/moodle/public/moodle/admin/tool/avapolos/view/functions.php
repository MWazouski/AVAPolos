  ...
  class Connect_SSH{
    ...
    public function exec_ssh_export(){
      ...
      if(ssh2_auth_password($this->connection, 'avapolos', 'avapolos')) { //CREDENCIAIS
        try{
          $stream = ssh2_exec($this->connection, 'cd '.$this->dirPath.'; bash '.$this->sh_file.' 1 '.$_SESSION["USER"]->username.' > '.$this->output_file);
        }catch(Exception $e){
          echo "Erro";
        }
    ...
    public function exec_ssh_import(){
      ...
      if(ssh2_auth_password($this->connection, 'avapolos', 'avapolos')) { //CREDENCIAIS
        try{
           $stream = ssh2_exec($this->connection, 'cd '.$this->dirPath.';bash '.$this->sh_file.' 2 '.$_SESSION["USER"]->username.' > '.$this->output_file);
        }catch(Exception $e){
          echo "Erro";
        }
        ssh2_exec($this->connection, 'exit');
