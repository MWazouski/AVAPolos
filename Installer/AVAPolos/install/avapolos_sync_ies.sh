...
dirLogPath="$dirPath/data/postgresql_master/pg_log"
...
while [ "$option" -ne "4" ]; do
  ...
  [1]* )
    ...
    if [ ! -f $configPath ]; then
      ...
      status=1
      pastaBackup="Backup_Import_"$status"_IES"
    else
    ...
    echo "Último export realizado IES: $status | Último sync IES-POLO realizado:$sync" > $configPath
    ...
    if cp -rf "$dirPath/dbdata1/" "$dirExportPath/$pastaBackup/database/"; then
  ...
  [2]* ) 
    ...
    for nameFile in $list; do
      ...
      if [ ! -f $configPath ]; then
        ...
        ### Ajustando o arquivo
        echo "Último export realizado IES: $status | Último sync IES-POLO realizado: $imp_number" > $configPath
        ...
        echo "REMOVING DBDATA2... | STATUS = [OK]"
        rm -rf "$dirPath/dbdata2"

        while [ -e $dirPath/dbdata2 ];
        do
        ...
        cp -R "$dirImportPath/$nameFile/database/dbdata2" "$dirPath/dbdata2"
        ...
        docker stop sync #master no polo eh o banco de sincronização - mudar nome do container de naster para IES e de sync para POLO
        while [ "$(docker inspect -f '{{.State.Running}}' sync)" == true ]; do          
